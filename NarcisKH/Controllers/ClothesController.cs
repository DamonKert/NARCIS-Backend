...
namespace NarcisKH.Controllers
{
	...
	public class ClothesController : ControllerBase
	{
		...
		{
			var cloth = await _context.Clothes.Include(x => x.Category).Include(x => x.Sizes).Include(x => x.Model).FirstOrDefaultAsync(x => x.Id == id);
		...
		public async Task<IActionResult> EditCloth([FromForm] UpdateClothRequest clothRequest)
		{
			...
			var cloth = _context.Clothes.Include(x => x.Sizes).Include(x => x.Model).Include(x=>x.Category).FirstOrDefault(x => x.Id == clothRequest.ID);
			if (cloth == null)
			{
				var notFoundResponse = new
				{
			...
			if (clothRequest.RemainingImages != null)
			{
			...
			if (cloth.Model != null && cloth.Model.Id != clothRequest.ModelId)
			{
				cloth.Model = _context.Models.FirstOrDefault(x => x.Id == clothRequest.ModelId);
			}
			if (clothRequest.SizeIDs.Count == 0)
			{
				var errorResponse = new
				{
					StatusCode = 400,
			...
			cloth.Sizes = new List<Size>();
			foreach (var sizeId in clothRequest.SizeIDs)
			{
				var size = _context.Sizes.FirstOrDefault(x => x.Id == sizeId);
				if (size == null)
				{
		...
		public async Task<IActionResult> DeleteCloth(int id)
		{
			...
			cloth.Sizes.Clear();
			await _context.SaveChangesAsync();
			cloth = await _context.Clothes.FindAsync(id);
		...
		public async Task<ActionResult> InitClothDate()
		{
			...
			//size.Clothes.Add(_context.Clothes.First(item=>item.Code == "TSHIRT"));
